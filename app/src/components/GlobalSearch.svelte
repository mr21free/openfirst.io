...
<style>
  ...
  /* app.css bumps the font to 16px here (anti-zoom) — trim the padding back
     so the box doesn't grow past its desktop size on top of that. A softer
     border also keeps the field from reading heavier than the bare icon
     buttons (.iconbtn) it sits next to in the mobile bar. */
  @media (max-width: 820px) { .gs-input { padding: 4px 10px 4px 30px; border-color: var(--rule-soft); } .gs-ico { left: 10px; } }
