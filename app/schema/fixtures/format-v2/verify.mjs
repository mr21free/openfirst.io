...
// example) on every format-v2 fixture: both must produce byte-identical
// recovered JSON — matching the committed expected.json — for every
// passphrase, and both must fail (not just one of them) on a wrong or
// missing passphrase. Mirrors format-v1/verify.mjs exactly; see that file
// for why there's no "unprotected" case here (v2 is protected-only).
//
// Run locally: node schema/fixtures/format-v2/verify.mjs
...
const expected = readFileSync(resolve(HERE, manifest.reference), 'utf8');
...
for (const fixture of manifest.fixtures) {
  for (const slot of fixture.slots) {
    ...
    if (res.status === 0) {
      ok(`${fixture.file} slot "${slot.label}": recovered data (incl. decrypted attachment) matches expected.json`, res.stdout === expected);
    }
  }
...
console.log('\n=== Format v2: recover.js vs recover.py ===');
let bad = 0;
